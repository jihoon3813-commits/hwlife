...
export default function Statistics() {
  ...
  return (
    <div className="p-8 space-y-6 overflow-y-auto no-scrollbar pb-20">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-[24px] font-bold text-[#191F28]">통계분석</h2>
          <p className="text-[13px] text-[#8B95A1] mt-1">실시간 방문자 및 신청 현황을 확인합니다. (Last Update: {new Date().toLocaleTimeString()})</p>
        </div>
        <div className="flex bg-white rounded-[12px] border border-[#E5E8EB] overflow-hidden p-1 shadow-sm">
          {['today', 'week', 'month'].map((p) => (
            <button 
              key={p}
              onClick={() => setPeriod(p)}
              className={`px-5 py-2 text-[13px] font-bold rounded-[8px] transition-all ${period === p ? 'bg-[#F2F4F6] text-[#191F28]' : 'text-[#4E5968] hover:bg-[#F9FAFB]'}`}
            >
              {p === 'today' ? '오늘' : p === 'week' ? '1주일' : '1개월'}
            </button>
          ))}
          <button className="px-5 py-2 text-[13px] font-medium text-[#4E5968] hover:bg-[#F9FAFB] border-l border-[#E5E8EB] ml-1">기간 설정</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        {statCards.map((card, idx) => {
          ...
          return (
            <div key={idx} className="bg-white p-7 rounded-[24px] border border-[#E5E8EB] shadow-sm hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-5">
                <div className={`p-3.5 rounded-[16px] ${card.bg}`}>
                  <Icon className={`w-6 h-6 ${card.color}`} />
                </div>
                <div className={`flex items-center gap-1 text-[13px] font-bold px-2.5 py-1 rounded-full ${isNeutral ? 'bg-gray-50 text-gray-400' : isPositive ? 'bg-green-50 text-green-500' : 'bg-red-50 text-red-500'}`}>
                  {isNeutral ? '' : isPositive ? <TrendingUp className="w-3 h-3"/> : <TrendingUp className="w-3 h-3 rotate-180"/>}
                  {card.change}
                </div>
              </div>
              <p className="text-[14px] font-bold text-[#4E5968] mb-1">{card.label}</p>
              <h3 className="text-[32px] font-black text-[#191F28]">{card.value}</h3>
            </div>
          );
        })}
      </div>

      <div className="bg-white p-8 rounded-[24px] border border-[#E5E8EB] shadow-sm">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-[18px] font-bold flex items-center gap-2 text-[#191F28]">
            <BarChart3 className="w-5 h-5 text-[#3182F6]" /> 최근 7일 통계
          </h3>
          <span className="text-[12px] text-[#8B95A1] font-medium">최근 일주일간의 데이터입니다.</span>
        </div>
        <div className="overflow-hidden rounded-[16px] border border-[#F2F4F6]">
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="bg-white p-8 rounded-[24px] border border-[#E5E8EB] shadow-sm">
          <h3 className="text-[17px] font-bold mb-6 text-[#191F28]">유입 경로 (Top 10 Referrer)</h3>
        ...
        <div className="bg-white p-8 rounded-[24px] border border-[#E5E8EB] shadow-sm">
          <h3 className="text-[17px] font-bold mb-6 text-[#191F28]">최근 접속 IP 현황</h3>
          <div className="overflow-hidden rounded-[16px] border border-[#F2F4F6]">
